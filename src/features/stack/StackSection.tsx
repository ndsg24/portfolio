...
import type { StackGroupId } from './stack.types'
...
const groupIcons = {
  ...
  data: DatabaseZap,
  operations: Activity,
} satisfies Record<StackGroupId, typeof Code2>
...
function StackSection() {
  const { stackGroups: groups, t } = usePortfolio()
  return (
    <m.section
      ...
      <m.div className="stack-board" variants={fadeUp}>
        ...
        <div className="stack-clusters">
          {groups.map((group) => {
            ...
            return (
              <m.article className="stack-cluster" key={group.id} variants={fadeUp}>
